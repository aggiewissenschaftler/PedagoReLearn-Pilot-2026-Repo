...
class PedagoReLearnEnvRewarded(gym.Env):
    ...
    def step(self, action: int):
        ...
        if verb == "teach" and idx is not None:
            ...
            sr = self._spacing_reward(idx);
            reward += sr;
            self._accum_spacing(sr)

        elif verb == "quiz" and idx is not None:
            rq, _ok = self._apply_quiz(idx);
            reward += rq;
            self._accum_quiz(rq)
            sr = self._spacing_reward(idx);
            reward += sr;
        ...
        # ✅ BUILD INFO DICT WITH SUCCESS INDICATORS
        info = {
            "all_mastered": done,  # ← CRITICAL: This is what the runner needs!
            "r_sums": dict(self.r_sums),  # Also helpful for diagnostics
        }
        ...
        return self._obs(), float(reward), done, truncated, info  # ← NOW RETURNS INFO!
